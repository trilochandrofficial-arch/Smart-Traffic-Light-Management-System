...
class Model(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions):
        ...
        self.device = torch.device("cSmoothL1uda" if torch.cuda.is_available() else "cpu")
        self.to(self.device)
...
class Agent:
    def __init__(
        ...
        # self.memory = dict()
        # for junction in junctions:
        #     # Replace existing memory initialization with PrioritizedExperienceReplay
        #     self.memory[junction] = PrioritizedExperienceReplay(
        #         capacity=max_memory_size,
        #         state_shape=(input_dims,),
        #         alpha=0.6,  # Set your desired prioritization hyperparameter
        #     )
    ...
    def store_transition(self, state, state_, action,reward, done,junction):
        ...
        # Pad or truncate the state to match the desired input_dims
        state = np.pad(state, (0, max(0, self.input_dims - len(state))))

        # Ensure the new state_ is a NumPy array
        state_ = np.array(state_)
        # Pad or truncate the state_ to match the desired input_dims
        state_ = np.pad(state_, (0, max(0, self.input_dims - len(state_))))
        ...
        # self.memory[junction].store(
        #     state, state_, reward, done, index, self.memory[junction].max_priority()
        # )
        # self.memory[junction]["mem_cntr"] += 1
    ...
    def choose_action(self, observation):
        ...
        # Ensure the size of the state tensor matches the input_dims
        state = F.pad(state, (0, max(0, self.input_dims - state.size(1))))
...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    if not train:
        base_path = os.getcwd()
        if 'DQN' not in base_path:
            base_path = os.path.join(base_path, 'DQN', 'models')
        elif 'models' not in base_path:
            base_path = os.path.join(base_path, 'models')

        model_name = model_name.strip()  # Remove leading and trailing spaces
        model_full_name = model_name + ".bin"
        model_path = os.path.join(base_path, model_full_name)
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    # Modify the state to be in the desired range [0, 12]
                    state = [max(0, min(x, 12)) for x in state]
                    state_ = [max(0, min(x, 12)) for x in state_]
